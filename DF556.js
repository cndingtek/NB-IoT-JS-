 ...
 * @Description: Demo code for pressure level sensor DF556 TCP Socket upload message parsing. Not including downlink command.
 */
//While integration with user system, please remark below.
//let payload = [0x80,0x00,0x16,0x03,0x20,0x04,0x05,0x00,0x1E,0x0A,0x1E,0x4B,0x1E,0x14,0x14,0x60,0x08,0x19,0x59,0x50,0x49,0x73,0x00,0x18,0x65,0x38,0x50,0x60,0x03,0x44,0x84,0x81,0xFF,0xFF,0xFF,0xFF,0xFF];
/************************************************************************************
 * 
...
switch (parseInt(payload[2]).toString(16)) {
    case '16':
        result = Parse_Code_DF556(payload, metadata);
        break;
    default:
    ...
    switch (payload_in[3].toString(16)) {
        ...
        case '3':
            if (payload_in.length >= parseInt(payload_in[4])) {
                ...
                var imsi="";
                for (var i = 0; i < 8; i++) {
                    imsi += LeftPadZero(parseInt(payload_in[14 + i]).toString(16), 2);
                }
                var upload_time = (parseInt(payload_in[7])<<8)+parseInt(payload_in[8]);                
                res = {
                    deviceName: deviceName.substring(1, 16),
                    deviceType: "DF556",
                    attributes: {
                        ...
                        detectInterval: parseInt(payload_in[9]), 
                        levelThreshold: parseInt(payload_in[10]),
                        batteryThreshold: parseInt(payload_in[13]),                                              
                        workMode: parseInt(payload_in[22]), 
                        imsi:imsi,
